...
void setup () {
  output = createWriter("./data/simulation_data.txt");
  size(500, 500);
  textSize(16);
...
void draw () {
  ...
  for (int i = 0; i <boids.size(); i++) {
    ...
    print(current.pos.x);
    boids_state = boids_state+"("+str(current.pos.x)+", "+str(current.pos.y)+")("+str(current.move.x)+", "+str(current.move.y)+")";
    print(boids_state);
    current.go();
    current.draw();
  }
  appendTextToFile("./simulation_data.txt", boids_state);
